...
export function rechargeCard(rechargeInfo) {
    return __awaiter(this, void 0, void 0, function () {
        var amount, cardId, card;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    amount = rechargeInfo.amount, cardId = rechargeInfo.cardId;
                    return [4 /*yield*/, cardRepository.findById(cardId)];
                case 1:
                    card = _a.sent();
                    if (amount < 1)
                        throwError("Insert a minimum amount of 1");
                    if (!card)
                        throwError("Card was not found!");
                    if (card.originalCardId)
                        throwError("You cannot recharge a virtual card");
                    if (!card.password)
                        throwError("You must activate your card");
